...
public final class VoltageAdapter extends ArrayAdapter<Voltage>
{
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent)
    {
        ...
        if (voltage.getType() == 0)
        {
            viewHolder.voltageSeekBarView.setProgress(voltage.getValue() - 700000);
        }
        else if (voltage.getType() == 1)
        {
            viewHolder.voltageSeekBarView.setProgress(voltage.getValue() * 1000 - 700000);
        }
        viewHolder.voltageSeekBarView.setOnSeekBarChangeListener(new OnSeekBarChangeListener()
        {
            ...
            public void onStopTrackingTouch(SeekBar seekBar)
            {
                ...
                if (voltage.getType() == 0)
                {
                    ...
                    new ChangeVoltage(getContext()).execute(new String[]{"singleseek", String.valueOf(prog + 700000), voltage.getFreq()});
                }
                else if (voltage.getType() == 1)
                {
                    VoltageAdapter.pd = ProgressDialog.show(VoltageAdapter.this.getContext(), null, VoltageAdapter.this.getContext().getResources().getString(R.string.changing_voltage), true, false);
                    new ChangeVoltage(getContext()).execute("singleseek", String.valueOf((prog + 700000) / 1000), position + "");
                }
            }
        ...
        if (voltage.getType() == 0)
        {
            viewHolder.buttonView.setText(String.valueOf((voltage.getVoltage()) / 1000) + "mV");
        }
        else if (voltage.getType() == 1)
        {
            viewHolder.buttonView.setText(String.valueOf((voltage.getVoltage())) + "mV");
        }
        ...
        viewHolder.freqView.setText(voltage.getFreq());
