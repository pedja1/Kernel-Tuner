...
public class FSHelper
{
    ...
    public static Frequency readFrequency(String path)
    {
        Frequency frequency = new Frequency();
        String freq;
        try
        {
            freq = FileUtils.readFileToString(new File(path)).trim();
        }
        catch(Exception e)
        {
            freq = "offline";
        ...
        {
            int freqVal = Tools.parseInt(freq);
            frequency.setFrequencyString(freqVal / 1000 + "Mhz");
            frequency.setFrequencyValue(freqVal);
        }
        else
        {
            frequency.setFrequencyString(freq);
            frequency.setFrequencyValue(-1);
        }
        return frequency;
    }


    public static List<Frequency> getAllCpuFreqs(boolean forceRead)
	{
		if (forceRead || MainApp.getInstance().getCpuFreqs().size() == 0)
		{
			...
			try
			{
				...
				for (String s : tmp2)
				{
                    ...
                    String freqH = freq / 1000 + "Mhz";
                    Frequency frequency = new Frequency();
                    frequency.setFrequencyValue(freq);
                    frequency.setFrequencyString(freqH);
					freqs.add(frequency);
				}
			}
			catch (IOException e)
	...
	public static int getCpuCount(int method)
	{
		int count = 0;
